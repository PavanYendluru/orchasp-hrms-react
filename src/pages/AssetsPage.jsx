...
export function AssetsPage() {
  ...
  return <div className="space-y-5"><PageHeader title="Assets" description="Track and manage company assets and inventory" actions={<div className="flex gap-2"><input ref={fileInput} type="file" accept=".csv,text/csv" className="hidden" onChange={importAssets} /><Button variant="outline" onClick={() => fileInput.current?.click()}><UploadFileOutlinedIcon className="h-4 w-4" /> Import CSV</Button><Button variant="outline" onClick={() => setAssignment({ assetId: '', employeeId: '' })}>Assign Asset</Button><Button onClick={() => setAssetForm({ ...blankAsset })}><AddOutlinedIcon className="h-4 w-4" /> Add Asset</Button></div>} />
    ...
    <AssetModal asset={assetForm} employees={data.employees} onClose={() => setAssetForm(null)} /><AssignmentModal assignment={assignment} assets={assets} employees={data.employees} onClose={() => setAssignment(null)} />
  </div>;
}

/** Collects the core inventory fields for both new and existing assets. */
function AssetModal({ asset, employees, onClose }) {
  const [values, setValues] = useState(asset || blankAsset);
  if (!asset) return null;
  const save = (event) => { event.preventDefault(); if (!values.name || !values.category || !values.serial || !values.value) return toast.error('Complete all asset fields.'); const updated = { ...values, value: Number(values.value) }; if (asset.id) { hrmsStore.assets.update(asset.id, updated); if (asset.status === 'available' && values.assignedToId) hrmsStore.assets.assign(asset.id, values.assignedToId); } else if (values.assignedToId) { hrmsStore.assets.create({ ...updated, status: 'assigned', assignedDate: new Date().toISOString().slice(0, 10) }); } else hrmsStore.assets.create(updated); toast.success(values.assignedToId ? 'Asset saved and assigned.' : asset.id ? 'Asset updated.' : 'Asset added.'); onClose(); };
  const selectedEmployee = employees.find((employee) => employee.id === asset.assignedToId);
  return <Modal open onOpenChange={(open) => !open && onClose()} title={asset.id ? 'Edit Asset' : 'Add Asset'} footer={<><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={save}>Save Asset</Button></>}><form className="grid gap-4 sm:grid-cols-2" onSubmit={save}>{[['Asset Name', 'name'], ['Asset Type / Category', 'category'], ['Serial Number', 'serial'], ['Value', 'value']].map(([label, name]) => <FormField key={name} label={label}><input className="input-base" type={name === 'value' ? 'number' : 'text'} value={values[name]} onChange={(event) => setValues({ ...values, [name]: event.target.value })} /></FormField>)}<FormField label="Status"><select className="input-base" value={values.status} onChange={(event) => setValues({ ...values, [name]: event.target.value })}>{Object.keys(statusVariant).map((status) => <option key={status} value={status}>{status}</option>)}</select></FormField>{asset.status === 'available' && <FormField label="Assign to employee"><select className="input-base" value={values.assignedToId || ''} onChange={(event) => setValues({ ...values, assignedToId: event.target.value })}><option value="">Keep available</option>{employees.filter((employee) => employee.status === 'active').map((employee) => <option key={employee.id} value={employee.id}>{employee.id} — {employee.firstName} {employee.lastName}</option>)}</select></FormField>}{asset.status === 'assigned' && <div className="rounded-lg border border-border bg-muted/40 p-3 text-sm"><p className="text-xs text-muted-foreground">Assigned employee</p><p className="mt-1 font-medium text-foreground">{selectedEmployee ? `${selectedEmployee.id} — ${selectedEmployee.firstName} ${selectedEmployee.lastName}` : asset.assignedToId}</p></div>}</form></Modal>;
}
